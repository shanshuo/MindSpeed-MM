...
def update_state_dict_inplace(
    ...
    for old_key in convert_mapping:
        ...
        if prefix + old_key in state_dict:
            count = count + 1
            if new_key in new_keys and "self_atten.proj_qkv" in new_key:
                state_dict[new_key] = torch.cat((state_dict[new_key], state_dict.pop(prefix + old_key)), dim=0)
            elif "scale_shift_table.1" in new_key:
                dict_old_value = state_dict.pop(prefix + old_key)
                if new_key not in state_dict:
                    chunk_total = [0] * 12
                    state_dict[new_key] = chunk_total
                if "norm1.linear" in old_key:
                    chunks_norm = torch.chunk(dict_old_value, 6, dim=0)
                    state_dict[new_key][0:3] = chunks_norm[0:3]
                    state_dict[new_key][6:9] = chunks_norm[3:6]
                if "norm2.linear" in old_key:
                    chunks_norm = torch.chunk(dict_old_value, 6, dim=0)
                    state_dict[new_key][3:6] = chunks_norm[0:3]
                    state_dict[new_key][9:12] = chunks_norm[3:6]
                temp = [isinstance(_, torch.Tensor) for _ in state_dict[new_key]]
                if all(temp):
                    state_dict[new_key] = torch.cat(state_dict[new_key], dim=0)
            else:
...
def get_layer_mapping_from_hf(i: int) -> Dict:
    layer_mapping = {}
    layer_mapping[f"transformer_blocks.{i}.norm1.linear.bias"] = f"videodit_blocks.{i}.scale_shift_table.1.bias"
    layer_mapping[f"transformer_blocks.{i}.norm2.linear.bias"] = f"videodit_blocks.{i}.scale_shift_table.1.bias"
    layer_mapping[f"transformer_blocks.{i}.norm1.linear.weight"] = f"videodit_blocks.{i}.scale_shift_table.1.weight"
    layer_mapping[f"transformer_blocks.{i}.norm2.linear.weight"] = f"videodit_blocks.{i}.scale_shift_table.1.weight"

    layer_mapping[f"transformer_blocks.{i}.attn1.norm_q.bias"] = f"videodit_blocks.{i}.self_atten.q_norm.bias"
    layer_mapping[f"transformer_blocks.{i}.attn1.norm_q.weight"] = f"videodit_blocks.{i}.self_atten.q_norm.weight"
    layer_mapping[f"transformer_blocks.{i}.attn1.norm_k.bias"] = f"videodit_blocks.{i}.self_atten.k_norm.bias"
    layer_mapping[f"transformer_blocks.{i}.attn1.norm_k.weight"] = f"videodit_blocks.{i}.self_atten.k_norm.weight"
    ...
    layer_mapping[f"transformer_blocks.{i}.ff.net.0.proj.bias"] = f"videodit_blocks.{i}.ff.net.0.proj.bias"
    layer_mapping[f"transformer_blocks.{i}.ff.net.0.proj.weight"] = f"videodit_blocks.{i}.ff.net.0.proj.weight"
...
def save_by_tp_pp(state_dicts: Dict[tuple, Dict], save_dir: str, enable_pp: bool,
...
def merge_by_tp_pp(train_save_dir: str, save_path: str, tp_size: int, pp_sizes: list):
    ...
    for pp_rank, _ in enumerate(pp_sizes):
        ...
        for tp_rank in range(tp_size):
            if len(pp_sizes) > 1:
                state_dict_path = os.path.join(train_save_dir, directory, f"mp_rank_{tp_rank:02d}_{pp_rank:03d}",
                                               "model_optim_rng.pt")
            else:
                state_dict_path = os.path.join(train_save_dir, directory, f"mp_rank_{tp_rank:02d}",
                                               "model_optim_rng.pt")
            _tp_state_dicts.append(torch.load(state_dict_path)['model'])
        is_last_pp_stage = pp_rank == len(pp_sizes) - 1
        merged_tp_state_dict = merge_by_tp(_tp_state_dicts, num_layers=pp_sizes[pp_rank], tp_size=tp_size,
...
if __name__ == "__main__":
    ...
    if args.mode == 'split':
        ...
        if args.task == "i2v":
            ...
            CONVERT_MAPPING_HF.update({"ofs_embedding.linear_1.weight": "ofs_embed.0.weight"})
            CONVERT_MAPPING_HF.update({"ofs_embedding.linear_2.bias": "ofs_embed.2.bias"})
            CONVERT_MAPPING_HF.update({"ofs_embedding.linear_2.weight": "ofs_embed.2.weight"})

        # inplace state dict
        for i in range(args.num_layers):
            CONVERT_MAPPING_HF.update(get_layer_mapping_from_hf(i))
        update_state_dict_inplace(source_state_dict, CONVERT_MAPPING_HF)

        # remove dummy layers
        remove_keys = set(source_state_dict.keys()) - set(CONVERT_MAPPING_HF.values())
        ...
        if sum(args.pp_sizes) != args.num_layers:
            raise ValueError(
                f"The sum of args.pp_sizes {args.pp_sizes} must be equal to args.num_layers {args.num_layers}")
