...
def extract_feature():
            ...
            for data_sample in train_dataset.data_samples:
                ...
                if source_data_storage_mode == "combine":
                    ...
                    raise NotImplementedError(
                        f"Extract features from data storage mode {source_data_storage_mode} is not implemented")
    ...
    for batch in train_dataloader:
        if batch:
            ...
            video_mask = batch.pop(VIDEO_MASK)
        ...
        if data_storage_mode == 'standard':
            ...
            print(
                f"[rank {torch.distributed.get_rank()}]:consumed sample {counter} | elapsed time {(time.time() - start_time):.2f} | file {pt_name}")

        elif data_storage_mode == 'sorafeatured':
            for i in range(bs):
                latent_i = latents[i].cpu()
                video_mask = video_mask[i].cpu()
